...
static bool CmpOptStringPtr(std::string * s1, std::string * s2) { 
  ...
  for(c1=0; c1 < (int)s1->size(); ++c1)
    if (isalnum(s[c1])) // locale sensitive.
      break;
      
  s=s2->c_str();
  for(c2=0; c2 < (int)s2->size(); ++c2)
    if (isalnum(s[c2]))
      break;
...
void ezOptionParser::reset() {
  ...
  for(i=0; i < (int)unknownArgs.size(); ++i)
    delete unknownArgs[i];
  unknownArgs.clear();
  
  for(i=0; i < (int)firstArgs.size(); ++i)
    delete firstArgs[i];
  firstArgs.clear();

  for(i=0; i < (int)lastArgs.size(); ++i)
    delete lastArgs[i];
  lastArgs.clear();
...
bool ezOptionParser::exportFile(const char * filename, bool all) {
  ...
  for(i=1; i < (int)firstArgs.size(); ++i) {
...
void ezOptionParser::prettyPrint(std::string & out) {
  ...
  for(i=0; i < n; ++i) {            
    ...
    out += "\n";
    // The flag names:
    for(j=0; j < (int)g->flags.size()-1; ++j) {
    ...
    if (g->isSet) {
      if (g->expectArgs) {
        if (g->args.empty()) {
          ...
          for(k=0; k < (int)g->args.size(); ++k) {            
            for(j=0; j < (int)g->args[k]->size()-1; ++j) {
