...
#define MB_LUA_FILEHANDLE "MBFILE*"
...
#define tolstream(L)	((LStream *)luaL_checkudata(L, 1, MB_LUA_FILEHANDLE))
...
static int io_type (lua_State *L) {
  LStream *p;
  luaL_checkany(L, 1);
  p = (LStream *)luaL_testudata(L, 1, MB_LUA_FILEHANDLE);
  if (p == NULL)
...
static LStream *newprefile (lua_State *L) {
  ...
  luaL_setmetatable(L, MB_LUA_FILEHANDLE);
  return p;
}
...
static int io_open (lua_State *l) {
	...
	{
		luaL_argcheck(l, false, 2, "valid file opening modes are \"r\" and \"b\"");
	}
  ...
  for (; nargs--; arg++) {
    if (lua_type(L, arg) == LUA_TNUMBER) {
      ...
      status = status &&
		...
		const int kMaxLineLength = 32*1024;
		char formattedLine[kMaxLineLength+2];
		...
		else
		{
			...
			{
				//Substitute line ending
				if (*srcCursor == '\n')
				{
					for (const char* lineEndingCursor = lineEnding; *lineEndingCursor; ++lineEndingCursor)
					{
						*dstCursor++ = *lineEndingCursor;
					}
				}
				else
				{
...
static void createmeta (lua_State *L) {
  luaL_newmetatable(L, MB_LUA_FILEHANDLE);  /* create metatable for file handles */
...
{
  luaL_newlib(L, mbfile);  /* new module */
  lua_setglobal(L, "mbfile");
  createmeta(L);
}
